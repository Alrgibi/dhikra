# Appendix B — The Libyan Pilot Protocol

This appendix reproduces the Libyan Arabic feasibility-study protocol, `docs/libyan_pilot_protocol.md` version 1.0, as committed in the project — the document prepared for submission to the University of Tripoli research ethics committee. The body text is the protocol's own; only the headings have been renumbered to this appendix's scheme and the tables captioned. One statement of context governs the reading of the whole document, and it is Chapter 6's ruled wording (section 6.3): contact with clinicians has been initiated, no arrangement exists, and securing a named clinician is the first step of the pathway and not yet done. Wherever the protocol refers to "the collaborating clinician" or "the collaborating clinical service", it therefore describes what would be required for the study to run, not an arrangement that exists; the reference-standard section (B.5) in particular is to be read in that conditional sense. The protocol's version and preparation line read: "Version 1.0** · Prepared for submission to the University of Tripoli research ethics committee and to the collaborating clinical service."

## B.1 What This Study Is And What It Deliberately Is Not
...
1. Can elderly Libyan participants **complete** the battery of four core assessment tasks plus one optional exploratory Qur'anic recitation probe?
...
17.00% in Israel to 6.86% in Egypt [1]. **Libya is not among the countries it reports.** A
...
published data"* [2] — and an
earlier systematic review of Arab-world dementia epidemiology searched for
Libyan studies and found none eligible [3]. **Libya's population prevalence of dementia
has never been characterised.** (Corrected: an earlier version of
...
Recruitment is specified in four parts: the target sample and its strata, the second role of the healthy stratum, the inclusion criteria and the exclusion criteria.

### B.3.1 Target Sample

The sample is defined by a minimum and a target, and by the two strata across which both are recruited; the healthy stratum carries a second role, set out in B.3.2, that determines its size.

**Minimum 20, target 40**, recruited across two strata:

Table (B.1) sets this out.

**Table (B.1) : Target sample by stratum**
...
### B.3.2 The Healthy Stratum Is Also The Normative Sample And That Changes Its Size

The main system's weakest reported figure is its 33.3%
specificity on the locked external corpus, and the analysis of sections 5.15 to 5.19 establishes that this was a *threshold referencing* failure rather than a model failure. The remedy is
...
Table (B.2) sets this out.

**Table (B.2) : Expected specificity of a control-referenced threshold by healthy sample size**
...
**Read this as a bound, not as an argument for a bigger study than can be run.** At the current target of 20 healthy participants the derived threshold
...
interval and 108 for a ±7.5-point one; those are the figures for a
...
population falls **below** the target rather than above it. That error direction favours sensitivity over missed cases, but excess false positives remain an equity, anxiety and resource-use concern. **Report it as a bounded, directional limitation
...
### B.3.3 Inclusion Criteria

Every participant must satisfy all of the following criteria at enrolment; each is checked by the clinician before consent is taken.
...
## B.4 Consent And The Capacity Question
...
**The form itself is drafted** at `docs/forms/CONSENT_FORM_ar.md`: information sheet and signature page as two separate documents, the
...
## B.6 Data Collected Per Participant

Three kinds of data are collected from every participant: the demographic fields, the recordings of the task battery, and the elicitation metadata that B.6.3 makes mandatory.

### B.6.1 Demographics And Why Every Field Is Collected

Each demographic field is collected for a stated analytical reason, and no field is collected without one. Table (B.3) sets this out.

**Table (B.3) : Demographic fields and the reason each is collected**
...
### B.6.2 The Task Battery And Why Its Ordering Was Revised
...
Table (B.4) sets this out.

**Table (B.4) : The task battery in administration order**
...
### B.6.3 Elicitation Must Be Fixed And This Is Not A Formatting Preference
...
## B.7 Recording Conditions

The recording conditions are fixed so that the acoustic measures describe the speaker and not the room; each condition below corresponds to a degradation the quality gate of the platform was tested against.
...
Data handling follows the rules below from the moment of recording; nothing leaves the device unencrypted and nothing identifiable leaves the clinic.
...
## B.9 Outcomes The Pilot Will Actually Report

The outcomes divide into the primary feasibility outcomes, the secondary normative and linguistic outcomes, two pre-registered secondary outcomes, and an explicit list of what is not an outcome; the subsections below take them in that order.

### B.9.1 Primary Outcomes Of Feasibility

The primary outcomes are the feasibility questions of B.1, each reported as a rate or a proportion by cognitive group.
...
### B.9.2 Secondary Outcomes Normative And Linguistic

The secondary outcomes are descriptive: they report what the measurements look like in the two strata, and they report them as distributions rather than as cut-offs.
...
### B.9.3 The Endpoint Decision For The Study This Pilot Is Preparing
...
1. **It is the only endpoint with evidence behind it.** The external validation is
   a dementia result — AUC 0.8533 [0.7371, 0.9458] on 53 recordings. On mild cognitive impairment the instrument is close to chance: the pooled development figure is 0.6291 [0.5703, 0.6868], and the Delaware Cookie Theft task alone returns 0.5061 in the task probe — which is why the battery below leads with connected discourse (section 5.25).
...
Table (B.5) sets this out.

**Table (B.5) : The two axes of the endpoint decision**
...
### B.9.4 What This Study Is Not Powered To Do
...
and sex-matched subset. **So the expectation for an Arabic dementia
...
*Two figures were corrected, and both were stale in the same
...
**Quantified.** At 20 per group, 80% power detects only a **true
AUC of 0.748 or larger** (Hanley–McNeil, two-sided α = 0.05).

Table (B.6) sets this out.

**Table (B.6) : Minimum detectable true AUC at 80% power by group size**
...
referential effect — they cost essentially the same.** *(Corrected. This previously read "75 per group for the MCI discourse effect", which
...
### B.9.5 The Scoring Rule Required On A Measured Result
...
absolute difference 1.600 against a threshold of 1.200; section 5.5.1). The
...
### B.9.6 A Preregistered Secondary Outcome On Whether The Substituted Picture Is Equivalent

**Added on a measured result.** The screening probability this
...
scene drawn for this project instead. The frozen model was then probed
...
### B.9.7 A Preregistered Secondary Outcome On The Minimal Probe

One outcome of this pilot *can* be a discriminative
...
**With the comparison stated in full.** Word count was selected as the strongest
...
### B.9.8 Explicitly Not An Outcome

The following are not outcomes of this study, and the analysis plan of B.10 does not compute them; any of them would require the larger study that B.9.4 sizes.
...
## B.10 Analysis Plan Fixed Before Collection

The analysis plan is fixed before the first participant is recorded, and it is deliberately short; the numbered steps below are the whole of it.
...
## B.11 What The Collaborating Clinician Is Being Asked For
...
## B.12 Realistic Timeline

Table (B.7) sets this out.

**Table (B.7) : Realistic timeline by stage**
